...
/* =====================================================
   v37.3 HARD FIX – Universal Document PDF/Image Studio
   Goal: Every document type can upload full PDF/image, select
   printable area by dragging, then generate A4 print-ready PDF.
   Unique function/class names avoid old cache conflicts.
===================================================== */
const V373_DOC_TYPES = {
  aadhaar:{label:'Aadhaar Card', icon:'🪪', w:85.6, h:54, ratio:85.6/54, hint:'Upload Aadhaar PDF/image. Drag the orange box over the card area.'},
  voter:{label:'Voter ID Card', icon:'🗳️', w:85.6, h:54, ratio:85.6/54, hint:'Upload downloaded voter card PDF/image and select the printable card.'},
  pan:{label:'PAN Card', icon:'💳', w:85.6, h:54, ratio:85.6/54, hint:'Upload PAN PDF/image and crop the card area for A4 printing.'},
  ayushman:{label:'Ayushman Card', icon:'❤️', w:85.6, h:54, ratio:85.6/54, hint:'Upload Ayushman card PDF/image and crop the front/back side.'},
  dl:{label:'Driving Licence', icon:'🚘', w:85.6, h:54, ratio:85.6/54, hint:'Upload driving licence PDF/image and select card area.'},
  abha:{label:'ABHA Card', icon:'🩺', w:85.6, h:54, ratio:85.6/54, hint:'Upload ABHA card PDF/image and crop the printable area.'}
};
let v373DocType='aadhaar';
let v373Slots={front:null, back:null};
function v373Hero(title,sub){return `<div class="studio-hero-v373"><div><span class="doc-badge-v373">v37.3 Document Studio</span><h2>${title}</h2><p>${sub}</p></div><div style="font-size:38px">🪪</div></div>`;}
function v373DocumentStudioTool(){
  const type=V373_DOC_TYPES[v373DocType]||V373_DOC_TYPES.aadhaar;
  workspace.innerHTML = `
    ${v373Hero('Universal Document Studio', 'Upload full document PDF or image, drag-select the printable card area, and generate an A4 print-ready PDF. Works for Aadhaar, Voter ID, PAN, Ayushman, Driving Licence and ABHA.')}
    <div class="doc-studio-v373">
      <div class="doc-toolbar-v373">
        <div><label>Document Type</label><select id="docTypeV373" onchange="v373ChangeDocType(this.value)">${Object.entries(V373_DOC_TYPES).map(([k,v])=>`<option value="${k}" ${k===v373DocType?'selected':''}>${v.icon} ${v.label}</option>`).join('')}</select></div>
        <div><label>Copies</label><select id="docCopiesV373"><option value="1">1 Copy</option><option value="2">2 Copies</option><option value="4">4 Copies</option><option value="6">6 Copies</option></select></div>
        <div><label>A4 Position</label><select id="docPosV373"><option value="top-center">Top Center</option><option value="top-left">Top Left</option></select></div>
        <div><label>Final Output</label><button class="primary-btn" onclick="v373CreateA4DocumentPDF()">Create A4 PDF</button></div>
      </div>
      <div class="doc-help-v373"><b>${type.icon} ${type.label}</b> — ${type.hint}<br><b>How to use:</b> upload the downloaded PDF/image → select page if needed → drag the orange printable area → resize using the corner dot → create A4 PDF. Final printed card size stays ${type.w}×${type.h} mm.</div>
      <div class="doc-crop-grid-v373">
        ${v373CropPanel('front','Front Side')}
        ${v373CropPanel('back','Back Side / Optional')}
      </div>
      <div id="docOutputV373"></div>
    </div>`;
  setTimeout(v373AttachInputs,60);
}
function v373CropPanel(slot,title){return `<div class="doc-crop-panel-v373"><h3>${title}</h3><label class="upload-box"><input id="docFile_${slot}_v373" type="file" accept="image/*,application/pdf"><span>📤 Upload Full PDF or Image</span></label><div id="docCropArea_${slot}_v373" class="doc-empty-v373">Upload a document PDF or image to start. Full PDF upload is supported; first page opens by default.</div></div>`;}
function v373ChangeDocType(type){v373DocType=type;v373Slots={front:null,back:null};v373DocumentStudioTool();}
function v373AttachInputs(){['front','back'].forEach(slot=>{const input=document.getElementById(`docFile_${slot}_v373`); if(input) input.onchange=e=>v373LoadFile(slot,e.target.files[0]);});}
async function v373LoadFile(slot,file){
  if(!file)return; const area=document.getElementById(`docCropArea_${slot}_v373`); if(!area)return; area.innerHTML='<div class="progress-box">Loading full document preview...</div>';
  const state={file, pages:[], currentPage:1, sourceCanvas:null, previewCanvas:null, box:null, drag:false, resize:false, startX:0, startY:0, start:{}}; v373Slots[slot]=state;
  try{
    if(file.type==='application/pdf' || file.name.toLowerCase().endsWith('.pdf')){
      if(!window.pdfjsLib) throw new Error('PDF library is not loaded. Refresh once with internet enabled.');
      const buf=await file.arrayBuffer(); state.pdf=await pdfjsLib.getDocument({data:buf}).promise; state.totalPages=state.pdf.numPages; await v373RenderPdfPage(slot,1);
    }else{
      const img=await loadImage(await readFile(file)); const c=document.createElement('canvas'); const ctx=c.getContext('2d'); c.width=img.width; c.height=img.height; ctx.fillStyle='#fff'; ctx.fillRect(0,0,c.width,c.height); ctx.drawImage(img,0,0); state.totalPages=1; v373ShowPreview(slot,c);
    }
  }catch(err){area.innerHTML=`<div class="warning-box">${err.message||err}</div>`;}
}
async function v373RenderPdfPage(slot,pageNo){
  const st=v373Slots[slot]; if(!st||!st.pdf)return; st.currentPage=pageNo; const page=await st.pdf.getPage(pageNo); const viewport=page.getViewport({scale:3}); const c=document.createElement('canvas'); const ctx=c.getContext('2d'); c.width=viewport.width; c.height=viewport.height; await page.render({canvasContext:ctx,viewport}).promise; v373ShowPreview(slot,c);
}
function v373ShowPreview(slot,sourceCanvas){
  const st=v373Slots[slot]; if(!st)return; st.sourceCanvas=sourceCanvas; const area=document.getElementById(`docCropArea_${slot}_v373`); const maxW=650; const ratio=sourceCanvas.width/sourceCanvas.height; const w=Math.min(maxW,sourceCanvas.width); const h=Math.round(w/ratio);
  const pageControls = st.totalPages>1 ? `<div class="doc-page-row-v373"><select id="docPage_${slot}_v373">${Array.from({length:st.totalPages},(_,i)=>`<option value="${i+1}" ${i+1===st.currentPage?'selected':''}>Page ${i+1} of ${st.totalPages}</option>`).join('')}</select><button onclick="v373ChangePage('${slot}')">Load Page</button></div>` : '';
  area.innerHTML = `${pageControls}<div class="canvas-box"><div class="doc-canvas-wrap-v373"><canvas id="docCanvas_${slot}_v373" class="doc-preview-v373"></canvas><div id="docBox_${slot}_v373" class="doc-crop-box-v373"><div id="docHandle_${slot}_v373" class="doc-crop-handle-v373"></div></div></div></div><div class="small-note">Drag the orange box to the exact printable document/card area. Use the corner dot to resize. The selected area will be placed on A4 at fixed card size.</div>`;
  const canvas=document.getElementById(`docCanvas_${slot}_v373`); const ctx=canvas.getContext('2d'); canvas.width=w; canvas.height=h; canvas.style.width=w+'px'; canvas.style.height=h+'px'; ctx.drawImage(sourceCanvas,0,0,w,h);
  const card=V373_DOC_TYPES[v373DocType]||V373_DOC_TYPES.aadhaar; let boxW=Math.round(w*0.82); let boxH=Math.round(boxW/card.ratio); if(boxH>h*0.78){boxH=Math.round(h*0.58);boxW=Math.round(boxH*card.ratio);} st.previewCanvas=canvas; st.box={x:Math.round((w-boxW)/2),y:Math.round((h-boxH)/2),w:boxW,h:boxH}; v373ApplyBox(slot); v373InitEvents(slot);
}
function v373ChangePage(slot){const sel=document.getElementById(`docPage_${slot}_v373`); if(sel) v373RenderPdfPage(slot,Number(sel.value));}
function v373ApplyBox(slot){const st=v373Slots[slot]; const el=document.getElementById(`docBox_${slot}_v373`); if(!st||!el||!st.box)return; el.style.left=st.box.x+'px'; el.style.top=st.box.y+'px'; el.style.width=st.box.w+'px'; el.style.height=st.box.h+'px';}
function v373Point(e){return e.touches&&e.touches[0]?{x:e.touches[0].clientX,y:e.touches[0].clientY}:{x:e.clientX,y:e.clientY};}
function v373InitEvents(slot){
  const st=v373Slots[slot]; const box=document.getElementById(`docBox_${slot}_v373`); const handle=document.getElementById(`docHandle_${slot}_v373`); const canvas=document.getElementById(`docCanvas_${slot}_v373`); if(!st||!box||!handle||!canvas)return;
  const start=(e,type)=>{e.preventDefault();e.stopPropagation();const p=v373Point(e); st.drag=type==='move'; st.resize=type==='resize'; st.startX=p.x; st.startY=p.y; st.start={...st.box};};
  box.onmousedown=e=>start(e,'move'); box.ontouchstart=e=>start(e,'move'); handle.onmousedown=e=>start(e,'resize'); handle.ontouchstart=e=>start(e,'resize');
  const move=e=>{if(!st.drag&&!st.resize)return; e.preventDefault(); const p=v373Point(e); const dx=p.x-st.startX; const dy=p.y-st.startY; const ratio=(V373_DOC_TYPES[v373DocType]||V373_DOC_TYPES.aadhaar).ratio; if(st.drag){st.box.x=st.start.x+dx; st.box.y=st.start.y+dy;} if(st.resize){let nw=Math.max(80,st.start.w+dx); let nh=Math.round(nw/ratio); st.box.w=nw; st.box.h=nh;} st.box.w=Math.min(st.box.w,canvas.width-st.box.x); st.box.h=Math.min(st.box.h,canvas.height-st.box.y); st.box.x=Math.max(0,Math.min(st.box.x,canvas.width-st.box.w)); st.box.y=Math.max(0,Math.min(st.box.y,canvas.height-st.box.h)); v373ApplyBox(slot);};
  const stop=()=>{st.drag=false; st.resize=false;}; document.addEventListener('mousemove',move); document.addEventListener('mouseup',stop); document.addEventListener('touchmove',move,{passive:false}); document.addEventListener('touchend',stop);
}
function v373Crop(slot){const st=v373Slots[slot]; const boxEl=document.getElementById(`docBox_${slot}_v373`); if(!st||!boxEl||!st.previewCanvas)return null; const rect=st.previewCanvas.getBoundingClientRect(); const b=boxEl.getBoundingClientRect(); const relX=(b.left-rect.left)/rect.width, relY=(b.top-rect.top)/rect.height, relW=b.width/rect.width, relH=b.height/rect.height; const sx=st.sourceCanvas.width*relX, sy=st.sourceCanvas.height*relY, sw=st.sourceCanvas.width*relW, sh=st.sourceCanvas.height*relH; const out=document.createElement('canvas'); const ctx=out.getContext('2d'); out.width=1200; out.height=Math.round(1200*sh/sw); ctx.fillStyle='#fff'; ctx.fillRect(0,0,out.width,out.height); ctx.drawImage(st.sourceCanvas,sx,sy,sw,sh,0,0,out.width,out.height); return out.toDataURL('image/jpeg',.95);}
async function v373CreateA4DocumentPDF(){
  const card=V373_DOC_TYPES[v373DocType]||V373_DOC_TYPES.aadhaar; const copies=Number(document.getElementById('docCopiesV373')?.value||1); const pos=document.getElementById('docPosV373')?.value||'top-center'; const srcs=[]; const front=v373Crop('front'); const back=v373Crop('back'); if(front)srcs.push(front); if(back)srcs.push(back); if(!srcs.length)return alert('Upload at least one PDF/image and select the printable area.');
  const {jsPDF}=window.jspdf; const pdf=new jsPDF({orientation:'portrait',unit:'mm',format:'a4'}); let y=10; const gap=8; for(let i=0;i<copies;i++){const totalW=srcs.length*card.w+(srcs.length-1)*gap; let x=pos==='top-left'?10:(210-totalW)/2; for(const s of srcs){pdf.addImage(s,'JPEG',x,y,card.w,card.h); pdf.setDrawColor(40); pdf.setLineWidth(.25); pdf.rect(x,y,card.w,card.h); x+=card.w+gap;} y+=card.h+8; if(y>250&&i<copies-1){pdf.addPage(); y=10;}}
  const url=URL.createObjectURL(pdf.output('blob')); let preview=''; for(let i=0;i<copies;i++){srcs.forEach(s=>preview+=`<img src="${s}">`);} const out=document.getElementById('docOutputV373'); if(out) out.innerHTML=`<div class="result-card fade-in"><h3>✅ ${card.label} A4 Print Ready</h3><p>Created from full PDF/image upload using the drag-selected printable area.</p><div class="action-row"><a class="open-btn" href="${url}" target="_blank">📂 Open PDF</a><a class="pdf-btn" href="${url}" download="${card.label.replace(/\s+/g,'-').toLowerCase()}-a4-print.pdf">📄 Download PDF</a><button class="print-btn" onclick="window.open('${url}','_blank')">🖨️ Print</button></div><div class="print-area ${pos}"><div class="doc-output-preview-v373">${preview}</div></div></div>`; if(typeof logToolUsage==='function') logToolUsage(card.label+' Document Studio v37.3',{toolType:'PDF'});
}

// Hard override: always route Document Studio to v37.3 engine.
(function(){
  const previousOpenTool = window.openTool || openTool;
  window.openTool = openTool = function(tool){
    if(tool==='documentStudio' || tool==='aadhaar') return v373DocumentStudioTool();
    if(tool==='imageStudio' && typeof imageStudioTool==='function') return imageStudioTool();
    if(tool==='pdfStudio' && typeof pdfStudioTool==='function') return pdfStudioTool();
    return previousOpenTool(tool);
  };
  window.home = home = function(){
    workspace.innerHTML=`${v373Hero('Smart Photo Toolkit Enterprise v37.3', 'Document Studio hard fix: full PDF/image upload, page selection, drag-select printable area, and A4 printing are now active for all supported document types.')}<div class="studio-grid-v37"><button class="studio-card-v37 primary-studio" onclick="openTool('imageStudio')"><b>🖼️</b><h3>Image Studio</h3><p>Passport Photo Studio, image compressor, resize, converter, and name/date tools.</p><span>Open Image Studio →</span></button><button class="studio-card-v37" onclick="openTool('documentStudio')"><b>🪪</b><h3>Document Studio</h3><p>Upload full PDF/image and drag-select printable area for Aadhaar, Voter ID, PAN, Ayushman, DL, and ABHA.</p><span>Open Document Studio →</span></button><button class="studio-card-v37" onclick="openTool('pdfStudio')"><b>📄</b><h3>PDF Studio</h3><p>PDF resize/compress is active.</p><span>Open PDF Studio →</span></button><button class="studio-card-v37" onclick="openTool('dashboard')"><b>👤</b><h3>Account Workspace</h3><p>Profile, membership, payments, and activity.</p><span>Open Account →</span></button></div><div class="v37-status"><h3>v37.3 Hard Fix</h3><p>If you still see an older version, clear browser site data or open with ?v=37.3-hard-fix.</p></div>`;
  };
  window.addEventListener('load',()=>setTimeout(()=>{try{home();}catch(e){}},900));
})();
